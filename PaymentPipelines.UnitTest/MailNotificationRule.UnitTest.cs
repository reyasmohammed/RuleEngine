...
{
    public class MailNotificationRuleUnitTest
    {
        [Fact(DisplayName = "Returns True - As the Product Type is Membership")]        
        public void MailNotificationRule_ShouldRun()
        {
            // Arrange
            var mockRule = new Mock<IRule<CartItemContext>>();
            mockRule.Setup(_ => _.CanExecute(It.IsAny<CartItemContext>())).Returns(false);

            var rule = new MailNotificationRule(mockRule.Object.Invoke);
        ...
        [Fact(DisplayName = "Returns False - As the Product Type is not Membership")]
        public void MailNotificationRule_ShouldNotRun()
        {
            // Arrange
            var mockRule = new Mock<IRule<CartItemContext>>();
            mockRule.Setup(_ => _.CanExecute(It.IsAny<CartItemContext>())).Returns(false);
